...
if cs is None:
    ...
    if srv.status == "ACTIVE":
        ...
        utils.log_msg(" ".join([server_log_id, "Bootstrapping Chef", ",Environment =", config.chef_env,]),
                      "INFO", config.print_to)
        utils.log_msg(" ".join([server_log_id, "Chef Roles =",config.chef_roles]),
                      "INFO", config.print_to)
                            ...
                            try:
                                ...
                                if skip_node:
                                    ...
                                    node = clb.Node(address=srv.networks["private"][0],
                                                ...
                                                condition="ENABLED")
                                    utils.log_msg(" ".join([server_log_id, "Adding server to load balancer"]),
                                                  "INFO",
                                                  config.print_to)
                                    attempts = 5
                                    while True:
...
utils.log_msg(" ".join([server_log_id, "Processing complete"]),
              "INFO",
              config.print_to)
